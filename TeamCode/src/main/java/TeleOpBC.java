...
Controls:
    ...
    GP 2:
    x --> intake
    Left Trigger -->  launch motors
    D-pad to turn/pitch turret
    Right trigger --> launch
    Right+Left Bumpers + b --> toggle auto
...
public class TeleOpBCFieldCentric extends OpMode {
    ...
    public void loop() {
        ...
        if(launch0.getPower() >= 0.9 && launch1.getPower() >= 0.9){
            rgb.setPosition(0.4994);
        } else{
            if(turretAuto){
                rgb.setPosition(0.2908);
            }else {
                rgb.setPosition(0.5745);
            }
        }
        ...
        //telemetry updates:)
        telemetry.addData("Launch power", (launch0.getPower()+launch1.getPower())/2.0);
        telemetry.addData("Turret Position (rotation)", turretPos / 10);
        telemetry.addData("rgb", rgb.getPosition());
        telemetry.addData("Auto?", turretAuto);
